...
class PickAndPlaceExperiment:
    ...
    def run_task(
        ...
        4. 작업자가 '조금' 올려/낮춰 달라고 요구할 경우 3~5cm(0.03~0.05m), '많이'라고 하면 10~15cm(0.10~0.15m) 내외로 계산하여 final_z_m을 결정하세요.
        5. 작업자의 음성이 맥락에 안 맞거나(예: "아 배고파", "어어") 알아들을 수 없으면 is_invalid를 true로 반환하고 높이를 유지하세요.
        6. 작업자가 "아파", "위험해", "멈춰", "그만" 등 비상 정지나 위험을 나타내는 말을 하면 is_emergency_stop을 true로 반환하세요.
        7. 작업자의 요구가 너무 모호하여 정확한 판단이 불가능하면 clarification_question에 작업자에게 되물을 질문을 한글로 작성하세요.
