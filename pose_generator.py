...
# 5차 측정에서 확보한 IK-feasible TCP pose lookup이다.
...
# 로봇 link0 기준 TCP z 사용 가능 범위다. 5차 측정 lookup 범위와 일치한다.
MIN_LINK0_Z_M = 0.051
MAX_LINK0_Z_M = 1.019
...
class HumanAwareTcpPoseGenerator:
    ...
    2. 로봇 모델: 5차 측정 IK-feasible pose lookup에서 H 기준 TCP pose를 보간한다.
    """
    ...
    def shoulder_angle_from_floor_height(
        ...
        value = (profile.shoulder_height_m - float(floor_height_m)) / profile.total_arm_length_m
    ...
    def interpolate_quaternion(self, target_floor_height_m: float) -> tuple[float, float, float, float]:
        """목표 높이 H에 가장 가까운 두 5차 측정 quaternion을 slerp로 보간한다."""
    ...
    def interpolate_measured_pose(
        ...
        """목표 높이 H에서 5차 측정 position과 orientation을 보간한다."""
